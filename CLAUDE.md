...
## CRITICAL: 도구 사용 가드레일 (코딩 에이전트 실행 안전)
- **네트워크 호출 금지**: `curl`, `wget` 등으로 외부 URL을 직접 호출하지 마라. 필요한 문서/코드는 이미 `.ai-factory/`와 `node_modules/`에 있다.
- **인라인 스크립트 금지**: `python3 -c ...` 같은 인라인 스크립트로 파일을 조작하지 마라.
- **일괄 치환 금지**: `sed`/`awk`로 파일을 일괄 치환하지 마라 — 의도치 않은 범위까지 바뀌어 리뷰가 불가능해진다.
- **파일 수정은 Read / Edit / Write 도구로만** 한다 — 셸 파이프로 파일을 덮어쓰지 마라.
- **아이콘은 `Asset.ContentIcon`만** 사용한다 — 직접 만든 SVG/이모지 아이콘 금지.
